...
subtitle: "Colorectal Cancer | Necroptosis"
author: 
...
Our genes of interest are MLKL (the primary executor of necroptosis) and the differentially expressed genes.

```{r}
for (project in projects) {
  for (gene in c("MLKL", deseq.bbl.data.combined$gene_symbol)) {
    cat(project, gene, "\n\n")
    gene_df <- construct_gene_df(gene, project)
    fit <- compute_surival_fit(gene_df)
    print(plot_survival(fit))
    print(compute_survival_diff(gene_df))
    cat("\n\n============================\n\n")
  }
}
