---
title: "Differential Gene Expression Analysis - TCGA - Pan-cancer - More RCD Types"
...
head(genes)
```
Filter the genes to include only those in the gene set of interest.
...
for (project in projects) {
  ...
  print(all(colnames(tcga_rcd[[project]]) == rownames((samples[[project]]))))
}
```
...
for (project in projects) {
  ...
  deseq.bbl.data[[project]] <- data.frame(
    ...
    cancer_type = project,
    gene_symbol = genes[rownames(deseq.results), "gene_symbol"]
  )
}
...
```
```{r}
rownames(deseq.results)
```
```{r}
genes[rownames(deseq.results), ]
```
